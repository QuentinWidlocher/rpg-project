...
export const fightingStyles = {
  ...
  fightingStyleDueling: {
    ...
    display: true,
    fn: () => ({ modifier: 2 }),
    predicate: (_props, _roll, item, character) => (item.type == 'weapon' && item.subType == 'melee' && !(item.tags.includes('two-handed')) && character.inventory.filter(item => item.type == "weapon" && item.equipped).length < 2),
    source: 'class',
    target: 'attackRoll',
    title: "Dueling",
    type: 'bonus',
  ...
  fightingStyleGreatWeaponFighting: {
    ...
    predicate: (_props, { roll }, weapon) => (weapon.tags.includes('two-handed') || weapon.tags.includes('versatile')) && roll <= 2,
    source: 'class',
    target: 'damageRoll',
  ...
  fightingStyleTwoWeaponFighting: {
    ...
    display: true,
    fn: (_props, _roll, _weapon, _actionCost, character) => ({ modifier: getProficiencyBonus(character) }),
    predicate: (_props, _roll, weapon, actionCost, character) => !weapon.tags.includes('two-handed') && actionCost == 'bonusAction' && character.inventory.filter(item => item.type == "weapon" && (item.tags.includes('one-handed') || item.tags.includes('versatile')) && item.equipped).length == 2,
    source: 'class',
    target: 'damageRoll',

...
export const fightingStyles = {
	...
	fightingStyleDefense: {
		...
		predicate: (_props, character) =>
			character.inventory.some(item => item.type == "armor" && item.equipped),
		source: "class",
		target: "armorClass",
		title: "Defense",
		type: "bonus",
	...
	fightingStyleDueling: {
		...
		predicate: (_props, _roll, item, character) =>
			...
			character.inventory.filter(item => item.type == "weapon" && item.equipped)
				.length <= 1,
		source: "class",
		target: "attackRoll",
		title: "Dueling",
	...
	fightingStyleGreatWeaponFighting: {
		...
		display: true,
		fn: (_props, _roll, weapon, _actionCost, character) =>
			getDamageRoll(weapon, character),
		// @FIXME: we should reroll a single die when needed
		predicate: (_props, { roll }, weapon) =>
			(weapon.tags.includes("two-handed") || weapon.tags.includes("versatile")) &&
			roll <= 2,
		source: "class",
		target: "damageRoll",
	...
	fightingStyleTwoWeaponFighting: {
		...
		predicate: (_props, _roll, weapon, actionCost, character) =>
			...
			character.inventory.filter(
				item =>
					item.type == "weapon" &&
					(item.tags.includes("one-handed") || item.tags.includes("versatile")) &&
					item.equipped,
			).length == 2,
		source: "class",
		target: "damageRoll",
		...
		fn: (_props, source) => {
			if (isStorePlayerCharacter(source)) {
				source.set("hp", "current", prev =>
					Math.min(prev + d10(1) + source.value.level, getMaxHp(source.value)),
				);
			}
		},

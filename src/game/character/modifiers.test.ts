...
describe('Basic modifiers', () => {
  ...
  describe(modifiers.fighterProficiencies.title, () => {
    test('should work with two fighters skills', () => {
      const isProficient = getModifierValue(
        [createModifierRef('fighterProficiencies', { skills: [fighterAvailableSkills[0], fighterAvailableSkills[1]] })],
        'skillProficiency',
        false,
      )
    ...
    test('should not work with two non fighters skills', () => {
      const isProficient = getModifierValue(
        [createModifierRef('fighterProficiencies', { skills: ['arcana', 'performance'] })],
        'skillProficiency',
        false,
      )
  ...
  describe(modifiers.baseSkillInitialValue.title, () => {
    test('should work with a base skill', () => {
      const getBaseSkill = getModifierValue(
        [createModifierRef('baseSkillInitialValue', { skill: 'charisma', value: 16 })],
        'baseSkill',
        10,
      )
  ...
  describe(modifiers.classWeaponProficiency.title, () => {
    test('should work with only simple weapons', () => {
      ...
      expect(getWeaponProficiency({ ...weapons[simpleWeapons[0]], equipped: true })).toBe(true)
      expect(getWeaponProficiency({ ...weapons[martialWeapons[0]], equipped: true })).toBe(false)
    })
  })
  ...
  describe(modifiers.classHitPoints.title, () => {
    ...
    test('should work for a lvl3 fighter with +2 const', () => {
      character.level = 3
      character.modifiers = [createModifierRef('baseSkillInitialValue', { skill: 'constitution', value: 14 })]

...
function createAdvantageToHitModifier(
	...
	} satisfies Partial<Modifier<{ timesToUse: number }, { timesUsed: number }>>;
	...
	if (target == "attackRoll") {
		return {
			...
			predicate: props => {
				return props.state.timesUsed < props.timesToUse;
			},
		} satisfies Modifier<{ timesToUse: number }, { timesUsed: number }>;
	} else {
		return {
			...base,
			target,
...
export function getModifierValue<Target extends Modifier["target"]>(
	...
	return (...args: ModifierFnParams<Target>): ModifierFnReturnType<Target> => {
		...
		if (lastOverride != null) {
			if (!lastOverride.props.state.markedAsDone && (!lastOverride.predicate || lastOverride.predicate(lastOverride.props, ...(args as [])))) {
		...
		for (const overrideBase of overrideBases) {
			if (overrideBase != null) {
				if (!overrideBase.props.state.markedAsDone && (!overrideBase.predicate || overrideBase.predicate(overrideBase.props, ...(args as [])))) {

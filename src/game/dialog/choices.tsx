...
export function getSkillCheckCondition(
  ...
  return { success: check.success, tooltip: `${getSkillLabel(chosenSkill)} : ${check.roll + check.modifier + check.proficiency} vs. ${dd}` } satisfies ReturnType<Choice['condition'] & {}>
}
...
export function skillCheckChoice(
  ...
  return {
    ...
    text: (props => (
      <>
        <span>
          {typeof choice.text == 'function' ? choice.text(props) : choice.text}
        </span>
        <div aria-disabled={!props.condition} class="ml-2 badge badge-outline badge-secondary aria-disabled:opacity-50"><IconoirFastArrowRight /><span class="mb-0.5">{getSkillLabel(chosenSkill)} : {dd}</span></div>
      </>
    )) satisfies Choice['text'],
    skillCheck: { character, skill: chosenSkill, dd, outcome: { success: choice.success, failure: choice.failure } } satisfies Choice['skillCheck'],
  };
}

...
export default function IntroDialog() {
  ...
  const [baseSkillValues, setBaseSkillValues] = createSignal<Record<BaseSkill, number>>({ strength: 10, dexterity: 10, wisdom: 10, charisma: 10, constitution: 10, intelligence: 10 })
  const [selectedSkills, setSelectedSkills] = createSignal<[Skill | undefined, Skill | undefined]>([undefined, undefined])
  const [selectedFightingStyle, setSelectedFightingStyle] = createSignal<keyof typeof fightingStyles | null>(null)

  const [equipment, setEquipment] = createStore<(null | ItemId)[][]>(new Array(classConfigs.fighter.startingEquipment.length).fill(null).map((_, i) => new Array(classConfigs.fighter.startingEquipment[i].length).fill(null)));
  const [choices, setChoices] = createStore<any>(new Array(classConfigs.fighter.startingEquipment.length).fill(null).map((_, i) => new Array(classConfigs.fighter.startingEquipment[i].length).fill(null).map((_, j) => new Array(classConfigs.fighter.startingEquipment[i][j].length ?? 1))));
  const [selectedChoices, setSelectedChoices] = createStore<any>(new Array(classConfigs.fighter.startingEquipment.length).fill(null));
    ...
    },
    {
      ...
      text: () => {
        function ValueSelector(props: { title: string, prop: BaseSkill }) {
          return <div class="flex-1 p-3 rounded-box bg-base-300 flex flex-col gap-2">
            ...
            <input
              ...
              onInput={e => {
                setBaseSkillValues(prev => ({ ...prev, [props.prop]: e.currentTarget.valueAsNumber }))
              }}
            />
        ...
        return <>
          Tell me about yourself <br /><br />
          <div class="flex flex-col gap-5 p-2">
            <div>
              What's your name ? <br />
              <input class="input w-full input-bordered" value={player.name} onInput={e => setPlayer('name', e.currentTarget.value)} />
            </div>
            <div>
              What's your class ? <br />
              <select class="w-full input-bordered select" value={player.class} onChange={e => setPlayer('class', e.currentTarget.value as Class)}>
                {classes.map(clazz => <option value={clazz} disabled={clazz != 'fighter'}>{getClassLabel(clazz)}</option>)}
              </select>
            </div>
          ...
          <div class="flex flex-col gap-5 p-2">
            <label class="form-control">
              ...
              <div class="flex gap-5">
                <select class="w-full input-bordered select" value={player.class} onChange={e => setSelectedSkills(prev => ([e.currentTarget.value as Skill, prev[1]]))}>
                  {classConfigs[player.class].availableSkills.map(skill => (
                    <option disabled={selectedSkills().includes(skill)} value={skill}>{getSkillLabel(skill)}</option>
                  ))}
                </select>
                <select class="w-full input-bordered select" value={player.class} onChange={e => setSelectedSkills(prev => ([prev[0], e.currentTarget.value as Skill]))}>
                  {classConfigs[player.class].availableSkills.map(skill => (
                    <option disabled={selectedSkills().includes(skill)} value={skill}>{getSkillLabel(skill)}</option>
                  ))}
                </select>
            ...
            <label class="form-control">
              ...
              <div class="join join-vertical">
                {(Object.entries(fightingStyles) as Array<[key: keyof typeof fightingStyles, Modifier]>).map(([key, mod]) => (
                  <div class="collapse join-item bg-base-100">
                    <input type="radio" name="fightingStyle" checked={key == selectedFightingStyle()} onChange={e => { if (e.currentTarget.checked) { setSelectedFightingStyle(key) } }} />
                    <div class="collapse-title text-xl font-medium pr-3">
                      <div class="flex justify-between items-center">
                        <span>{mod.title} {key == selectedFightingStyle()} </span>
    ...
    },
    {
      ...
      text: () => (<>
        ...
        <ul class="not-prose">
          {classConfigs.fighter.startingEquipment.map((choice, i) => (
            <>
              {i > 0 ? <div class="divider divider-primary"><span class="text-primary text-lg"><IconoirPlus /></span></div> : null}
              <li>
                <ul class="flex flex-col gap-2">
                  {choice.map((itemIds, j) => (
                    <li class="form-control">
                      <label class="btn bg-base-100 justify-start flex-nowrap">
                        <input class="radio radio-sm radio-primary -ml-1 mr-1" type="radio" name={`choice-${i}`} checked={selectedChoices[i] == j} onChange={(e) => {
                          setEquipment(i, itemIds.map((itemId, k) => Array.isArray(itemId) ? choices[i][j][k] : itemId));
                          setSelectedChoices(i, j);
                        }} />
                        {itemIds.map((itemId, k) => (
                          <>
                            {k > 0 ? (<li><IconoirPlus /></li>) : null}
                            <li class="shrink">
                              {Array.isArray(itemId) ? (
                                <select class="select select-sm select-primary w-full" onChange={e => {
                                  ...
                                  {itemId.map(id => (<option value={id}>{items[id].name}</option>))}
                                </select>
      ...
      exitFunction: (props) => {
        if (equipment.flat().some(e => e == null)) {
          ...
          setPlayer('inventory', (prev) => [...prev, ...equipment.flat().filter(Boolean).map(e => ({ ...items[e], equipped: false }))])
        }
      }
    },
    {

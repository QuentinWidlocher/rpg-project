...
export function detailedSkillCheck(character: PlayerCharacter, skill: BaseSkill | Skill, dd: number) {
  ...
  return { roll, modifier, proficiency, success: roll + modifier + proficiency >= dd } satisfies DetailedSkillCheckResult
}
...
export function pickBestSkill(character: PlayerCharacter, skill: (BaseSkill | Skill) | Array<BaseSkill | Skill>): BaseSkill | Skill {
  return Array.isArray(skill) ?
    skill.reduce(({ name: bestSkillName, value: bestSkillValue }, currentSkillName) => {
    ...
    }, { name: skill[0], value: 0 } as { name: BaseSkill | Skill, value: number }).name
    : skill
}

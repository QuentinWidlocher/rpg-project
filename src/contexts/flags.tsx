...
export type FlagsContext = { getFlag: (flagName: FlagName) => boolean, setFlag: (flagName: FlagName, value?: boolean) => void }

export const FlagsContext = createContext<FlagsContext>();

export function FlagsProvider(props: ParentProps) {
  const [flags, setFlags] = makePersisted(createStore<Flags>({}), { name: 'flags' })
  ...
  return (
    <FlagsContext.Provider value={{ getFlag, setFlag }}>
      {props.children}

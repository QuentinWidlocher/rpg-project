...
export function DiceThrow(props: {
  ...
  createEffect(on(() => props.values, function roll() {
    if (props.values) {
          ...
          <div class="card">
            <div class="card-body bg-base-100 rounded-xl text-center">
              ...
              <span class="countdown text-6xl mx-auto mt-2" >
                <span style={{ "--value": rollingResult().details?.hitRoll + rollingResult().details.hitModifier }} />
              </span>
              <span>vs.</span>
              <span class="text-6xl">
                {values().details?.defenderAC}
              </span>
              {rollDone() ? (
                <button onClick={() => props.onClose()} class={`btn mt-5 btn-wide ${values().success ? 'btn-success' : 'btn-error'}`}>
                  {values().success ? (
                    <span class="countdown flex gap-1">Hit ! <span style={{ '--value': hitResult() }} /> damage inflicted</span>
                  ) : 'Miss...'}
                </button>

    ...
    >
      <div id="left">
        <div class="tooltip tooltip-bottom before:z-30" data-tip="Your Character">
          <button
            onClick={() => open(() => <CharacterModal />)}
            class="btn btn-circle btn-sm btn-ghost m-1 text-2xl"
          >
            <IconoirUser />
          </button>
        </div>
      ...
      <div id="center" class="w-1/2">
        <div class="tooltip tooltip-top after:opacity-0! w-full">
          <div class="tooltip-content" style="--tt-pos: 35px !important">{player.hp.current}/{maxHp()} HP</div>
          <progress class={twJoin("progress w-full", props.transparent ? 'progress-secondary' : 'text-white')} value={player.hp.current} max={maxHp()}></progress>
        </div>
      </div>
      <div id="right">

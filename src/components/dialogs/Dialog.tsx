...
export function DialogComponent(props: { dialog: Dialog }) {
  ...
  createEffect(on(currentScene, function onSceneChange() {
    if (currentScene()) {
  ...
  return (
    <Show when={currentScene()} fallback={<Navigate href="/" />}>
      {
        (currentScene) => (
          <Layout
            illustration={illustration().background || illustration().character ? (
              <div class="grid grid-cols-2 grid-rows-1 @container h-full">
                ...
                {illustration().character ? (<img class="row-start-1 row-span-1 col-start-1 col-span-2 @sm:col-start-2 @sm:col-end-2 @sm:col-span-1 w-full h-full object-contain object-bottom px-3 pt-8 pb-0" src={illustration().character!} />) : null}
              </div>
            ) : undefined}
            title={currentScene().title(immutableFunctionProps())}
          >
            <DialogText

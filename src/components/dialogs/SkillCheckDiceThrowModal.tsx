...
export function SkillCheckDiceThrowModal(props: {
  onClose: () => void;
  values: SkillCheckProps | null;
}) {
          ...
          <div class="card w-96">
            <div class="card-body bg-base-200 rounded-xl text-center">
              ...
              <button
                ...
                class={`aria-hidden:opacity-0 transition-opacity btn mt-5 mx-auto btn-wide ${total() > fixedValues().dd ? "btn-success" : "btn-error"
                  }`}
              >
                {total() > fixedValues().dd ? (
                  "Embrace the success !"
                ) : (
                  "Endure the failure..."
                )}
